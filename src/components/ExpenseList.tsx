...
export default function ExpenseList({
  ...
  // Calculate total amount for current page expenses
  ...
  if (expenses.length === 0 && isFirstPage) {
    return (
    ...
    <div className="space-y-4">
          ...
          {(hasMore || !isFirstPage) && (
            <div className="flex items-center gap-2 bg-gray-100 dark:bg-white/10 p-1 rounded-xl border border-gray-200 dark:border-white/20 w-full md:w-auto justify-center">
              ...
              <button
                onClick={onNextPage}
                disabled={!hasMore}
                className={`p-2 rounded-lg transition-colors ${!hasMore
                  ? "text-gray-400 cursor-not-allowed"
                  : "text-gray-700 hover:bg-white dark:text-white dark:hover:bg-white/10"
                  }`}
              >
      ...
      {expenses.length === 0 ? (
        <div className="bg-white border border-gray-200 dark:bg-white/10 dark:backdrop-blur-xl dark:border-white/20 rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-red-300">
            Try clearing filters or selecting a different time period
          </p>
        </div>
      ) : (
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl overflow-hidden">
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-white/10">
                {expenses.map((expense) => (
                  <tr
                    key={expense.id}
                    className={`transition-all duration-150 hover:bg-slate-50 dark:hover:bg-white/5 ${editingId === expense.id
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-right">
                      {editingId === expense.id ? (
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${expense.editCount >= 2
          ...
          <div className="md:hidden divide-y divide-gray-200 dark:divide-white/10">
            {expenses.map((expense) => (
              <div
                key={expense.id}
                className={`p-4 transition-all duration-150 ${editingId === expense.id
                  ? "bg-indigo-50 border-l-4 border-indigo-500 dark:bg-indigo-500/10 dark:border-indigo-400"
                  : ""
                  }`}
              >
                  ...
                  <>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        onClick={() => startEdit(expense)}
                        disabled={expense.editCount >= 2}
                        className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${expense.editCount >= 2
